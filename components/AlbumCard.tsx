...
interface AlbumCardProps {
  ...
  onDragStart: (e: React.DragEvent, id: string, source: 'pool' | 'ranked') => void;
}
...
export const AlbumCard: React.FC<AlbumCardProps> = ({
  ...
  return (
    <div
      draggable
      onDragStart={(e) => onDragStart(e, album.id, isRanked ? 'ranked' : 'pool')}
      className={`
    ...
    >
      ...
      {isRanked && (
        <div className="flex-shrink-0 w-8 h-8 flex items-center justify-center bg-green-500/10 text-green-400 font-bold rounded-full text-sm border border-green-500/20">
          {rank}
        </div>
      )}
      ...
      <div className="flex-1 min-w-0">
        <h4 className="font-semibold text-white truncate text-sm md:text-base">{album.title}</h4>
